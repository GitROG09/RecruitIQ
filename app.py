...
def run_pipeline(jd_text: str, candidates_data: list, weights_override: dict[str, float]):
    ...
    # 4. Resolve effective weights for this run.
    #    NOTE: we deliberately do NOT mutate config.scoring.MATCH_WEIGHTS
    #    in place. Streamlit Community Cloud serves every user session
    #    from the same Python process, so patching a module-level global
    #    here would leak one user's slider weights into another user's
    #    concurrent ranking run. Weights are passed explicitly instead.
    effective_weights = {**config.scoring.MATCH_WEIGHTS, **weights_override}
    ...
    for i, candidate in enumerate(candidates):
        ...
        result = matcher.match(candidate, fv, job, weights=effective_weights)
...
def main():
    ...
    if run_pressed:
        ...
        # Execute pipeline
        try:
            ranked, paths, p_time, c_size, cand_dict = run_pipeline(
                jd_text, candidates_data, weights_override
            )
        except (KeyError, TypeError, ValueError) as exc:
            st.error(
                "Couldn't process the uploaded candidate data. Please check "
                "that the JSON file matches the expected schema "
                f"(see `data/raw/sample_candidates.json` for reference). "
                f"Details: {exc}"
            )
            st.stop()
