...
class OutputWriter:
    ...
    def __init__(self, output_dir: Path | None = None) -> None:
        ...
        output_dir : Path | None
            Target directory. Defaults to a unique subfolder of
            ``OUTPUT_DIR`` (from settings), namespaced with a UUID.

            A fresh, unique subfolder per run/instance matters once this
            is deployed: Streamlit Community Cloud serves every user
            session from one shared process and filesystem. Writing
            every run to the same fixed ``ranked_candidates.csv`` path
            means two users clicking "Analyze" around the same time can
            overwrite each other's export mid-download, or one user's
            download button could serve another user's data.
        """
        base_dir = Path(output_dir) if output_dir else OUTPUT_DIR
        self.output_dir = base_dir / uuid.uuid4().hex
