...
def render_candidate_detail(selected_candidate):
    ...
    for entry in res.reasoning:
        summary_sentences.append(str(entry["reason"]))
    # Escaped because it's rendered further down via unsafe_allow_html,
    # and skill/matcher reasons can be influenced by user-uploaded
    # candidate/JD data.
    ai_summary = html.escape(" ".join(summary_sentences))
    ...
    if skill_entry and "evidence" in skill_entry:
        ...
        matched = [html.escape(str(s)) for s in evidence.get("matched_skills", [])]
        missing = [html.escape(str(s)) for s in evidence.get("missing_skills", [])]
