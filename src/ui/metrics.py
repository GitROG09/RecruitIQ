...
def render_top_candidate_spotlight(results, candidate_dict):
    ...
    # NOTE: this data comes from a user-uploaded JSON file. It is rendered
    # below via unsafe_allow_html, so it MUST be HTML-escaped first or a
    # malicious upload (e.g. anonymized_name = "<img src=x onerror=...>")
    # could execute arbitrary script in the browser (stored XSS).
    full_cand = candidate_dict.get(top_cand.candidate_id)
    name = html.escape(str(getattr(full_cand.profile, 'anonymized_name', top_cand.candidate_id) if full_cand else top_cand.candidate_id))
    role = html.escape(str(getattr(full_cand.profile, 'current_title', 'Unknown Role') if full_cand else 'Unknown Role'))
    company = html.escape(str(getattr(full_cand.profile, 'current_company', 'Unknown Company') if full_cand else 'Unknown Company'))
    ...
    strengths_html = "".join([f"<li style='margin-bottom: 0.5rem;'><strong style='color:#10b981;'>{html.escape(str(r['matcher']).capitalize())}</strong>: {html.escape(str(r['reason']))}</li>" for r in top_strengths])
    gap_html = f"<li><strong style='color:#ef4444;'>{html.escape(str(primary_gap['matcher']).capitalize())}</strong>: {html.escape(str(primary_gap['reason']))}</li>"
    
    recommendation = f"Excellent fit for this role with strong semantic alignment ({top_cand.semantic_score:.1f}%) and proven capabilities across {html.escape(str(top_strengths[0]['matcher']))} and {html.escape(str(top_strengths[1]['matcher']))} dimensions. Minor gap identified in {html.escape(str(primary_gap['matcher']))}."
